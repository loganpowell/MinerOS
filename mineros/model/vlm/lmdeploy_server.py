...
from mineros.backend.vlm.utils import set_lmdeploy_backend
...
def main():
    ...
    device_type = os.getenv("MINEROS_LMDEPLOY_DEVICE", device_type)
    if device_type == "":
        device_type = "cuda"
    elif device_type not in ["cuda", "ascend", "maca", "camb"]:
        raise ValueError(f"Unsupported lmdeploy device type: {device_type}")
    lm_backend = os.getenv("MINEROS_LMDEPLOY_BACKEND", lm_backend)
    if lm_backend == "":
