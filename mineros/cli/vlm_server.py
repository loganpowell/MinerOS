...
def vllm_server():
    from mineros.model.vlm.vllm_server import main

    main()


def lmdeploy_server():
    from mineros.model.vlm.lmdeploy_server import main
...
def openai_server(ctx, inference_engine):
    ...
    if inference_engine == "auto":
        try:
            ...
            logger.info(
                "vLLM not found, attempting to use LMDeploy as the inference engine for VLM server."
            )
            try:
                import lmdeploy
                ...
                logger.error(
                    "Neither vLLM nor LMDeploy is installed. Please install at least one of them."
                )
                sys.exit(1)
    ...
    if inference_engine == "vllm":
        try:
            ...
            logger.error(
                "vLLM is not installed. Please install vLLM or choose LMDeploy as the inference engine."
            )
            sys.exit(1)
        vllm_server()
        ...
        try:
            ...
            logger.error(
                "LMDeploy is not installed. Please install LMDeploy or choose vLLM as the inference engine."
            )
            sys.exit(1)
        lmdeploy_server()
