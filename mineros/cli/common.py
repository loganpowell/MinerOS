...
from mineros.utils.draw_bbox import draw_layout_bbox, draw_span_bbox
from mineros.utils.engine_utils import get_vlm_engine
from mineros.utils.enum_class import MakeMode
from mineros.utils.guess_suffix_or_lang import guess_suffix_by_bytes
from mineros.utils.pdf_image_tools import images_bytes_to_pdf_bytes
from mineros.backend.vlm.vlm_middle_json_mkcontent import union_make as vlm_union_make
from mineros.backend.office.office_middle_json_mkcontent import (
    union_make as office_union_make,
)
from mineros.backend.vlm.vlm_analyze import doc_analyze as vlm_doc_analyze
from mineros.backend.vlm.vlm_analyze import aio_doc_analyze as aio_vlm_doc_analyze
from mineros.backend.office.docx_analyze import office_docx_analyze
...
def _process_output(
    ...
    if f_draw_layout_bbox:
        try:
            ...
            logger.warning(
                f"Skipping layout bbox visualization for {pdf_file_name}: {exc}"
            )
    ...
    if f_draw_span_bbox:
        try:
            ...
            logger.warning(
                f"Skipping span bbox visualization for {pdf_file_name}: {exc}"
            )
    ...
    if process_mode == "pipeline" and pdf_bytes:
        try:
            ...
            logger.warning(f"Strikethrough tagging failed: {exc}")

    if process_mode == "vlm" and pdf_bytes:
        try:
            from mineros.utils.strikethrough_utils import (
                substitute_strikethrough_in_vlm_pdf_info,
            )

            substitute_strikethrough_in_vlm_pdf_info(pdf_info, pdf_bytes)
        except Exception as exc:
            logger.warning(f"Strikethrough substitution failed: {exc}")
    ...
    with ThreadPoolExecutor(max_workers=1) as output_executor:
        ...
        def on_doc_ready(doc_index, model_list, middle_json, ocr_enable):
            ...
            future = output_executor.submit(
                run_output_task, doc_index, middle_json, model_list
            )
