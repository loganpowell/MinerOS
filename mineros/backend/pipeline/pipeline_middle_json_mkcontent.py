...
from mineros.utils.char_utils import full_to_half_exclude_marks, is_hyphen_at_line_end
from mineros.utils.config_reader import get_latex_delimiter_config
from mineros.backend.pipeline.para_split import ListLineTag
from mineros.utils.enum_class import BlockType, ContentType, ContentTypeV2, MakeMode
from mineros.utils.language import detect_lang
from mineros.backend.utils.markdown_utils import escape_conservative_markdown_text
...
def merge_visual_blocks_to_markdown(para_block, img_buket_path=""):
    ...
    for block in get_blocks_in_index_order(para_block.get("blocks", [])):
        ...
        rendered_segments.extend(
            render_visual_block_segments(render_block, img_buket_path)
        )
...
def _render_span(span, escape_markdown=True):
    ...
    content = _apply_span_style(span, content)
    return span_type, content


def _apply_span_style(span, content: str) -> str:
    """Wrap *content* in markdown style markers from span['style']."""
    styles = span.get("style", [])
    if not styles:
        return content
...
def _join_rendered_span(
    para_block, block_lang, line, line_idx, span_idx, span_type, content
):
...
def _split_list_item_blocks(para_block):
    ...
    for line_idx, line in enumerate(para_block.get("lines", [])):
        if (
            line_idx > 0
            and line.get(ListLineTag.IS_LIST_START_LINE, False)
            and current_lines
        ):
            item_blocks.append(
                {
    ...
    if para_type in [
            ...
            if block["type"] == BlockType.TABLE_BODY:
                for line in block["lines"]:
                    for span in line["spans"]:
                        if span["type"] == ContentType.TABLE:
                            if span.get("html", ""):
                                para_content[BlockType.TABLE_BODY] = (
                                    _format_embedded_html(
                                        span["html"],
                                        img_buket_path,
                                    )
                                )
        ...
        for block in para_block.get("blocks", []):
            if block["type"] == BlockType.CHART_BODY:
                for line in block["lines"]:
                    for span in line["spans"]:
                        if span["type"] == ContentType.CHART and span.get(
                            "image_path", ""
                        ):
                            para_content["img_path"] = (
                                f"{img_buket_path}/{span['image_path']}"
            ...
            if block["type"] == BlockType.CHART_CAPTION:
                para_content[BlockType.CHART_CAPTION].append(
                    merge_para_with_text(block)
                )
            if block["type"] == BlockType.CHART_FOOTNOTE:
                para_content[BlockType.CHART_FOOTNOTE].append(
                    merge_para_with_text(block)
                )
    elif para_type == BlockType.CODE:
        para_content = {
        ...
        for block in para_block.get("blocks", []):
            ...
            if block["type"] == BlockType.CODE_FOOTNOTE:
                para_content[BlockType.CODE_FOOTNOTE].append(
                    merge_para_with_text(block)
                )
