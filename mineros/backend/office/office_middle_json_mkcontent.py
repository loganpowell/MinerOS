...
from mineros.utils.enum_class import MakeMode, BlockType, ContentType, ContentTypeV2
...
OFFICE_STYLE_RENDER_MODE_ENV = "MINEROS_OFFICE_STYLE_RENDER_MODE"
...
def _join_rendered_parts(parts: list[dict]) -> str:
    ...
    for i, part in enumerate(parts):
        ...
        if span_type == ContentType.INLINE_EQUATION:
            ...
            if prev_part is not None and _needs_markdown_it_boundary_space(
                prev_part, part
            ):
                para_text += " "
            para_text += content
...
def _append_hyperlink_part(
    ...
    if plain_text_only:
        leading = original_content[
            : len(original_content) - len(original_content.lstrip())
        ]
...
def make_blocks_to_content_list(para_block, img_buket_path, page_idx):
    ...
    if para_type in [
        ...
        _attribute = para_block.get("attribute", "unordered")
        para_content = {
            "type": para_type,
    ...
    for page_info in pdf_info_dict:
        ...
        if make_mode in [MakeMode.MM_MD, MakeMode.NLP_MD]:
            ...
            page_markdown = mk_blocks_to_markdown(
                paras_of_layout, make_mode, img_buket_path, page_idx=page_idx
            )
