...
执行该命令后，您将进入到Docker容器的交互式终端，并映射了一些端口用于可能会使用的服务，您可以直接在容器内运行MinerOS相关命令来使用MinerOS的功能。
...
我们提供了[compose.yml](https://github.com/loganpowell/MinerOS/blob/main/docker/compose.yaml)文件，您可以通过它来快速启动MinerOS服务。
...
> [!NOTE]
>
> - `compose.yaml`文件中包含了MinerOS的多个服务配置，您可以根据需要选择启动特定的服务。
...
> - 如果您希望聚合已有的 `mineros-api` 服务而不是启动本地 worker，可直接参考 `compose.yaml` 中 `mineros-router` 服务下的注释示例，改为使用 `--upstream-url`。
