# MinerOS 扩展模块安装指南
MinerOS 支持根据不同需求，按需安装扩展模块，以增强功能或支持特定的模型后端。

## 常见场景

### 核心功能安装
`core` 模块是 MinerOS 的核心依赖，包含了除`vllm`/`lmdeploy`外的所有功能模块。安装此模块可以确保 MinerOS 的基本功能正常运行。
```bash
...
如果您需要在边缘设备上安装轻量版的 client 端以连接兼容 openai 接口的服务端来使用vlm模式，可以安装mineros的基础包，非常轻量，适合在只有cpu和网络连接的设备上使用。
```bash
...
如果您需要在边缘设备上安装轻量版的 client 端以连接兼容 openai 接口的服务端来使用hybrid模式，可以安装mineros的pipeline扩展包，相对较轻量，可以在只有cpu和网络连接的设备上使用，同时在支持gpu加速的设备上可以更快运行。
```bash
