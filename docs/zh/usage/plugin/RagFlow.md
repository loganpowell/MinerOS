...
目前，MinerOS 已深度集成至 RAGFlow 知识库在线版本，作为内置 PDF 文档解析器，为用户知识库搭建提供专业、可靠的文档解析支持。本地部署版本部署使用方式详见下方使用教程。

使用可访问：https://demo.ragflow.io/

![img](../../../assets/images/RagFlow_01.png)

## 使用教程：如何在 RAGFlow 中使用 MinerOS

### 一、安装配置

首先，我们建议您通过 docker 的形式在本地部署 RagFlow 以方便使用 MinerOS 插件作为解析工具。在安装完 RagFlow 后执行：
...
2. **更新 .env 文件：**
   ...
   MINEROS_EXECUTABLE=/ragflow/uv_tools/.venv/bin/mineros
   ```
...
4. **在容器内下载 MinerOS 模型：**
   ...
   uv pip install -U "mineros[core]" -i https://mirrors.aliyun.com/pypi/simple
   ```
...
5. **退出并重启：**
   ...
   运行重启命令，让 RAGFlow 加载刚装好的 MinerOS
...
在本地部署完毕后，要启用 MinerOS，您需要在进入 RagFlow 特定知识库的配置页面并选择 MinerOS 作为默认的 PDF 解析器。（注：RagFlow 在线版中已经内置了 MinerOS 插件为您提供了高级的 PDF 文件解析能力，使用方式与此一致。）
