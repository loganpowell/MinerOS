...
MinerOS默认使用`huggingface`作为模型源，若用户网络无法访问`huggingface`，可以通过环境变量便捷地切换模型源为`modelscope`：
...
MinerOS内置了命令行工具，用户可以通过命令行快速使用MinerOS进行文档解析：
...
- 通过fast api方式调用：
  ...
  > - 可通过环境变量 `MINEROS_API_TASK_RETENTION_SECONDS` 和 `MINEROS_API_TASK_CLEANUP_INTERVAL_SECONDS` 调整保留时长与清理轮询间隔。
  ...
  > ```
  >
  > http异步调用代码示例：[Python版本](https://github.com/loganpowell/MinerOS/blob/main/demo/demo.py)
...
- 启动gradio webui 可视化前端：
  ...
  > - 未传 `--api-url` 时，Gradio 会自动拉起可复用的本地 `mineros-api`；传入 `--api-url` 时则会复用已有本地或远端服务。
  > - `--enable-vlm-preload true` 会让 Gradio 在 WebUI 启动阶段主动拉起本地 `mineros-api` 并等待 VLM 预加载完成；传入 `--api-url` 时会被忽略。
  > - WebUI 当前支持上传 `PDF`、图片与 `DOCX` 文件。

- 通过 `mineros-router` 进行多服务 / 多 GPU 编排：
  ...
  > [!TIP]
  >
  > - `mineros-router` 对外暴露与 `mineros-api` 一致的 `/health`、`/tasks`、`/file_parse`、`/tasks/{task_id}`、`/tasks/{task_id}/result` 接口。
  > - 可重复使用 `--upstream-url` 聚合多个已有 `mineros-api` 服务，也可通过 `--local-gpus` 自动拉起本地 worker。
...
- 使用`http-client/server`方式调用：
  ...
  > `hybrid-http-client` 需要本地具备 `mineros[pipeline]` 及 `torch` 等 pipeline 依赖。

> [!NOTE]
> 所有`vllm/lmdeploy`官方支持的参数都可用通过命令行参数传递给 MinerOS，包括以下命令:`mineros`、`mineros-openai-server`、`mineros-gradio`、`mineros-api`、`mineros-router`，
> 我们整理了一些`vllm/lmdeploy`使用中的常用参数和使用方法，可以在文档[命令行进阶参数](./advanced_cli_parameters.md)中获取。

## 基于配置文件扩展 MinerOS 功能

MinerOS 现已实现开箱即用，但也支持通过配置文件扩展功能。您可通过编辑用户目录下的 `mineros.json` 文件，添加自定义配置。

> [!IMPORTANT]
> `mineros.json` 文件会在您使用内置模型下载命令 `mineros-models-download` 时自动生成，也可以通过将[配置模板文件](https://github.com/loganpowell/MinerOS/blob/main/mineros.template.json)复制到用户目录下并重命名为 `mineros.json` 来创建。
...
- `models-dir`：
  ...
  - 指定目录后您可通过配置环境变量`export MINEROS_MODEL_SOURCE=local`来使用本地模型。
