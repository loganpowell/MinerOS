...
```bash
wget https://raw.githubusercontent.com/loganpowell/MinerOS/main/docker/china/kxpu.Dockerfile
docker build --network=host -t mineros:kxpu-vllm-latest -f kxpu.Dockerfile .
```
...
docker run -u root --name mineros_docker \
    ...
    -e MINEROS_FORMULA_CH_SUPPORT=true \
    -e MINEROS_VLLM_DEVICE=kxpu \
    -it mineros:kxpu-vllm-latest \
    /bin/bash
```
...
不同环境下，MinerOS对Kunlunxin加速卡的支持情况如下表所示：
...
>[!TIP]
> - Kunlunxin加速卡指定可用加速卡的方式与NVIDIA GPU类似，请参考[使用指定GPU设备](https://loganpowell.github.io/MinerOS/zh/usage/advanced_cli_parameters/#cuda_visible_devices)章节说明,
