...
```bash
wget https://raw.githubusercontent.com/loganpowell/MinerOS/main/docker/china/mlu.Dockerfile
docker build --network=host -t mineros:mlu-lmdeploy-latest -f mlu.Dockerfile .
```
...
docker build --network=host -t mineros:mlu-vllm-latest -f mlu.Dockerfile .
```

## 3. 启动 Docker 容器

```bash
docker run --name mineros_docker \
   --privileged \
   --ipc=host \
   --network=host \
   --shm-size=400g \
   ...
   -e MINEROS_LMDEPLOY_DEVICE=camb \
   -it mineros:mlu-lmdeploy-latest \
   /bin/bash
```
...
>
> - 替换上述命令中的`mineros:mlu-lmdeploy-latest`为`mineros:mlu-vllm-latest`
>
...
>[!NOTE]
> **兼容性说明**：由于寒武纪（Cambricon）目前对 vLLM v1 引擎的支持尚待完善，MinerOS 现阶段采用 v0 引擎作为适配方案。
> 受此限制，vLLM 的异步引擎（Async Engine）功能存在兼容性问题，可能导致部分使用场景无法正常运行。
> 我们将持续跟进寒武纪对 vLLM v1 引擎的支持进展，并及时在 MinerOS 中进行相应的适配与优化。

不同环境下，MinerOS对Cambricon加速卡的支持情况如下表所示：
...
>[!TIP]
> - Cambricon加速卡指定可用加速卡的方式与NVIDIA GPU类似，请参考[使用指定GPU设备](https://loganpowell.github.io/MinerOS/zh/usage/advanced_cli_parameters/#cuda_visible_devices)章节说明,
