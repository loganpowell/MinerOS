...
>   ```bash
>   CUDA_VISIBLE_DEVICES=1 mineros -p <input_path> -o <output_path>
>   ```
> - This method works for all command-line invocations, including `mineros`, `mineros-openai-server`, `mineros-gradio`, `mineros-api`, and `mineros-router`, and it applies to both the `pipeline` and `vlm` backends.
...
>   ```bash
>   # In terminal 1
>   CUDA_VISIBLE_DEVICES=0 mineros-openai-server --engine vllm --port 30000
>   # In terminal 2
>   CUDA_VISIBLE_DEVICES=1 mineros-openai-server --engine vllm --port 30001
>   ```
> 
...
>   ```bash
>   # In terminal 1
>   CUDA_VISIBLE_DEVICES=0 mineros-api --host 127.0.0.1 --port 8000
>   # In terminal 2
>   CUDA_VISIBLE_DEVICES=1 mineros-api --host 127.0.0.1 --port 8001
>   ```
> 
...
>   ```bash
>   CUDA_VISIBLE_DEVICES=0,1,2,3 mineros-router --host 127.0.0.1 --port 8002
>   ```
